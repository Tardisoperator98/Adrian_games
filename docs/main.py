...
snake_pos = [100, 60]
snake_kropp = [[100, 60], [100-40, 60], [100-(2*40), 60]]
...
bakgrunn = pygame.image.load('chrisbakgrunn.jpg')
bakgrunn = pygame.transform.scale(bakgrunn, (bredde, høyde))

snake_hode_bilde = pygame.image.load('chris2.png')
snake_hode_bilde = pygame.transform.scale(snake_hode_bilde, (30, 30))

snake_kropp_bilde = pygame.image.load('cottonswab.jpg')
snake_kropp_bilde = pygame.transform.scale(snake_kropp_bilde, (20, 20))

mat_bilde = pygame.image.load('cartpen.png')
mat_bilde = pygame.transform.scale(mat_bilde, (30, 30))

søppel_bilde = pygame.image.load('epleskrott.png')
søppel_bilde = pygame.transform.scale(søppel_bilde, (30, 30))
...
def spill_meny(): 
    ...
    start_meny_text = start_meny_font.render('Aala sitt snake-spill!', True,hvit )

    start_knapp_font = pygame.font.SysFont('time new roman', 50)
    start_knapp_text = start_knapp_font.render('START SPILLET', True, hvit)
    start_knapp = start_knapp_text.get_rect(center=(390, 380))

    regler_font = pygame.font.SysFont('time new roman', 30)
    regler_tekst = regler_font.render("Et terapautisk spill for å komme over eksen!", True, hvit)
...
bakgrunn_spill = pygame.image.load('gress.jpg')
bakgrunn_spill = pygame.transform.scale(bakgrunn_spill, (bredde, høyde))

while kjør: 

    pygame.display.update()

    spillvindu.blit(bakgrunn_spill, (0, 0))
